...
import { earthHelioVector } from './sun.js';
...
export function moonIlluminationAtTT(tt: number): MoonIllumination {
    ...
    // `GeoVector(body, time, aberration=false)`: the Moon is unaffected by the
    // flag (moonGeoVectorEqj above); the Sun reduces to -earthHelioVector(tt)
    // because HelioVector(Sun, t) is always the origin, so BackdatePosition's
    // aberration=false branch (a fixed, non-backdated observer position) never
    // iterates.
    const sunEqj: Vec3 = { x: -earth.x, y: -earth.y, z: -earth.z };
    const moonLonDeg = eclipticLonOfDateDeg(gc, tt);
    const sunLonDeg = eclipticLonOfDateDeg(sunEqj, tt);
    const phase = normalizeLongitude(moonLonDeg - sunLonDeg) / 360;
